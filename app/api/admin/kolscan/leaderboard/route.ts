...
export async function GET(request: NextRequest) {
  const limited = rateLimit({ request, key: "admin:kolscan:leaderboard", limit: 30, windowMs: 60_000 })
  ...
  try {
    ...
    if (!result.ok) {
      return NextResponse.json({ ok: false, error: "Kolscan fetch failed", details: result.error, status: result.status }, { status: 502 })
    }
